	...
	stages {
		stage('Build code') {
			steps{
				...
				sh './compile_app.sh'
				sh 'rm compile_app.sh && rm compile_${Language}_app.sh'
			}	
        	}
		stage('Create docker image'){
        	...
        	stage('Test: Code quality and security') {
				...
				cp $JENKINS_HOME/workspace/TFG-WORKSPACE/ScriptsJenkinsTFG/Test/test_${Language}_app.sh && \
				chmod 700 test_app.sh && \
                      		chmod 700 test_${Language}_app.sh'
                                sh './test_app.sh'
                                sh 'rm test_app.sh && rm test_${Language}_app.sh'
            		}
        	}
